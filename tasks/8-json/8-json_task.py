"""
Script Name: 8-json_task.py
Purpose: An interactive Object-Oriented Python application stored inside '8-json'.
         Supports manual input, batch text processing, automated CSV analytics reporting,
         and new JSON batch data ingestion via JsonFileReader.
"""
...
# -------------------------------------------------------------------------
# Path Configuration & Module-Level Constants (All local to 8-json & project root)
# -------------------------------------------------------------------------
...
# Files stored locally in 8-json or shared project root
...
class Publication:
    ...
    def publish(self, filename: str = OUTPUT_FILENAME) -> None:
        ...
        # Trigger automatic CSV analytics update (The Ripple Effect)
...
# -------------------------------------------------------------------------
# JSON Batch Reader Class (New in Module 8)
# -------------------------------------------------------------------------
class JsonFileReader:
    """Handles reading, parsing, publishing, and cleanup of multi-record JSON input files."""

    def __init__(self, file_path: str = ""):
        self.file_path = file_path.strip() if file_path else DEFAULT_JSON_FILE

    def process_file(self) -> None:
        """Loads JSON file, iterates through records, publishes them, and deletes the file."""
        if not os.path.exists(self.file_path):
            print(f"\n[Error] Sorry, file not found at '{self.file_path}'. Please check the path and try again.")
            return

        print(f"\n[Processing] Reading JSON file from: {self.file_path}")
        ...
        try:
            ...
            if not isinstance(records, list):
                print("[Error] Invalid JSON structure. Root element must be a list of objects.")
                return
            ...
            for record in records:
                ...
                if rec_type == "news":
                    ...
                    if text and exp_date:
                        ...
                        print("[Warning] Skipping Private Ad record due to missing fields.")

                elif rec_type in ["sports frequence", "sports", "sport"]:
                    text = record.get("text", "")
                    freq = record.get("frequency_per_week", None)
                    if text and isinstance(freq, int):
                        SportsFrequence(text, freq).publish()
                        success_count += 1
                    else:
                        print("[Warning] Skipping Sports record due to missing text or invalid frequency.")
                else:
                    print(f"[Warning] Unknown record type encountered: '{record.get('type')}'")

            # Cleanup: Delete file if successfully processed
            if success_count > 0:
                os.remove(self.file_path)
                print(
                    f"\n[Success] JSON batch processing complete! Published {success_count} record(s) and deleted '{self.file_path}'.")
            else:
                print("\n[Notice] No valid records were processed. File was not deleted.")

        except json.JSONDecodeError as jde:
            print(
                f"\n[Error] JSON Syntax Error: Failed to parse file due to invalid formatting (e.g., missing comma or bracket). Details: {jde}")
        except Exception as e:
            print(f"\n[Error] An unexpected error occurred while processing the JSON file: {e}")
...
def main():
    print("========================================")
    print("    NEWS FEED & API JSON INGESTION      ")
    print("========================================")
    ...
    while True:
        ...
        print("4 - Process records from JSON file (API Batch)")
        ...
        if choice == '0':
            ...
            path_input = input(f"Enter JSON file path (Press Enter for default): ").strip()
